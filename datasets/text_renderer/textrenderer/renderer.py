...
import datasets.text_renderer.libs.math_utils as math_utils
from datasets.text_renderer.libs.utils import draw_box, draw_bbox, prob, apply
from datasets.text_renderer.libs.timer import Timer
from datasets.text_renderer.textrenderer.liner import Liner
from datasets.text_renderer.textrenderer.noiser import Noiser
import datasets.text_renderer.libs.font_utils as font_utils

# noinspection PyMethodMayBeStatic
from datasets.text_renderer.textrenderer.remaper import Remaper


class Renderer(object):
    def __init__(self, corpus, fonts, bgs, cfg, width=256, height=32,
                 clip_max_chars=False, debug=False, gpu=False, strict=False, font_unsupport_chars=None):
        self.corpus = corpus
        self.fonts = fonts
        self.bgs = bgs
        ...
        if self.strict:
            self.font_unsupport_chars = font_unsupport_chars

    def gen_img(self, img_index):
        word, font, word_size = self.pick_font(img_index)
        self.word = word
        self.word_size = word_size
        #print(word, font, word_size)
        #print(word, word_size, font)
        ...
        if bg is None:
            print('word:', word, 'font:',font, 'word_size:',word_size)
        ...
        # cv2.imshow('ttt', word_img.astype(np.uint8))
        # cv2.waitKey()
    ...
    def crop_img(self, img, text_box_pnts_transformed):
        ...
        dst_height = random.randint(self.out_height // 4 * 3, self.out_height)  # 放缩到 out_height * 0.75~1.0

        # compute scale
        if self.out_width == 0:
            scale = bbox_height / dst_height
        else:
            dst_width = self.out_width  # 放缩到 out_width
            scale = max(bbox_height / dst_height, bbox_width / self.out_width)  # dst 放缩系数

        # compute scaled bbox width and height
        s_bbox_width = math.ceil(bbox_width / scale)    # 等比例放缩到 dst_width
        s_bbox_height = math.ceil(bbox_height / scale)  # 等比例放缩到 dst_height

        if self.out_width == 0:
            padding = random.randint(s_bbox_width // 10, s_bbox_width // 8)  # padding = (0.1~0.125) * s_bbox_width
            dst_width = s_bbox_width + padding * 2  # random pad to width
        ...
        # final cropped text image, It's important do crop first and than do resize for speed consider
        dst = img[dst_bbox[1]:dst_bbox[1] + dst_bbox[3], dst_bbox[0]:dst_bbox[0] + dst_bbox[2]]


        if dst is None:
            print('-------1: ', dst_bbox, dst_width, self.out_height)

        # resize to 
        try:
            dst = cv2.resize(dst, (dst_width, self.out_height), interpolation=cv2.INTER_CUBIC)
        except:
            print('------------------', dst_bbox, img.shape, dst_width, self.out_height, self.word, self.word_size)
        
        '''

        # --------------- if no random enlarge -----------------
        # minx = int(min(text_box_pnts_transformed[:,0]))
        # miny = int(min(text_box_pnts_transformed[:,1]))
        # maxx = int(max(text_box_pnts_transformed[:,0]))
        # maxy = int(max(text_box_pnts_transformed[:,1]))
        # dst_bbox = (minx, miny, maxx-minx, maxy-miny) 
        # dst = img[miny:maxy, minx:maxx]

        # or 
        box = cv2.boundingRect(text_box_pnts_transformed)
        dst_bbox = (box[0], box[1], box[2], box[3])

        # 安装 y 方向进行 random space
        offsetty = random.randint(-int(box[3]*0.1), int(box[3]*0.15))
        offsetby = random.randint(-int(box[3]*0.1), int(box[3]*0.15))
        scale = 1.0 * self.out_height / (box[3] + offsetty + offsetby)
        total_offsetx = int(self.out_width / scale) - box[2]
        if total_offsetx >= 0:
            #print('-----------------', 1)

            tmpmin = min(- int(0.1*box[3]), total_offsetx + int(0.1*box[3]))
            tmpmax = max(- int(0.1*box[3]), total_offsetx + int(0.1*box[3]))
            offsetlx = random.randint(tmpmin, tmpmax)
            offsetrx = total_offsetx - offsetlx
        else:
            #print('-----------------', 2)

            # 按照 x 方向进行 random space
            offsetlx = random.randint(-int(box[3]*0.1), int(box[2]*0.2))
            offsetrx = random.randint(-int(box[3]*0.1), int(box[2]*0.2))
            scale = 1.0 * self.out_width / (box[2] + offsetlx + offsetrx)
            total_offsety = int(self.out_height / scale) - box[3]

            tmpmin = min(- int(0.1*box[3]), total_offsety + int(0.1*box[3]))
            tmpmax = max(- int(0.1*box[3]), total_offsety + int(0.1*box[3]))
            offsetty = random.randint(tmpmin, tmpmax)
            offsetby = total_offsety - offsetty

        dst_bbox = (box[0]-offsetlx, box[1]-offsetty, box[2]+offsetrx+offsetlx, box[3]+offsetby+offsetty)
        
        # scale = 1.0 * self.out_height / dst_bbox[3]
        # dst_width = int(dst_bbox[2] * scale)
        # offset_width = int((self.out_width - dst_width) / scale)
    ...
    def gen_bg_from_image(self, width, height):
        ...
        # out = cv2.resize(bg, None, fx=scale, fy=scale)  # close bg scaling
        out = bg
        if out is None:
        ...
        #out = self.apply_gauss_blur(out, ks=[3, 5])  #, 7, 11, 13, 15, 17
        if out is None:
            print('height:', height, 'width:', width, x_offset, y_offset)        
        ...
        if self.strict:
            ...
            for c in word:
                ...
                if c in unsupport_chars:
                    # use fonts that have no unsupport chars
                    font_path = random.choice(['./datasets/text_renderer/data/fonts/chn/SourceHanSans-Normal.ttf',
                                               './datasets/text_renderer/data/fonts/chn/msyh.ttc'])
